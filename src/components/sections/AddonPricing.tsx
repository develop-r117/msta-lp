...
const paidAddons: PaidAddon[] = [
  { name: "決済", price: "680", description: "決済基盤の有効化・特商法対応", note: "決済手数料 2% / 出金手数料 3.6%" },
  { name: "メール配信", price: "980", description: "メールキャンペーンの作成・配信" },
  { name: "問い合わせフォーム", price: "980", description: "問い合わせフォームの管理" },
  ...
  { name: "マップコンテンツ", price: "980", description: "位置情報コンテンツの管理" },
  { name: "アプリ内課金・会員ランク", price: "980", description: "アプリ内課金（IAP）と会員ランク別アクセス制御" },
  { name: "チャット", price: "1,480", description: "ユーザーとのリアルタイムチャット" },
  { name: "EC", price: "1,980", description: "ECサイト・商品管理", note: "決済手数料 2% / 出金手数料 3.6%" },
  { name: "予約", price: "1,980", description: "定期・イベント予約／スタッフ管理／Googleカレンダー連携／事前決済", note: "予約手数料 2% / 出金手数料 3.6%" },
];
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-14 overflow-hidden rounded-3xl border border-neutral-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-neutral-200 bg-neutral-50 px-6 py-4">
            <span className="text-sm font-semibold text-neutral-800">有償アドオン</span>
            <span className="text-xs font-medium text-neutral-500">月額（税抜） / 初期費用 0円</span>
          </div>
            ...
            {paidAddons.map((addon, i) => (
              ...
              >
                <div className="min-w-0">
                  <p className="text-sm font-bold text-neutral-900">{addon.name}</p>
                  <p className="mt-0.5 text-xs leading-relaxed text-neutral-500">{addon.description}</p>
                  {addon.note ? (
                ...
                <p className="flex shrink-0 items-baseline gap-1 sm:justify-end">
                  <span className="text-2xl font-black text-neutral-900">¥{addon.price}</span>
                  <span className="text-xs font-semibold text-neutral-500">円 / 月</span>
                </p>
              </motion.li>
            ))}
          </ul>
        ...
        >
          <div className="flex items-center gap-2">
            <span className="rounded-full bg-primary-100 px-3 py-1 text-xs font-bold text-primary-700">無料アドオン</span>
            <span className="text-xs text-neutral-500">追加料金なしで有効化できます</span>
          </div>
