    ...
    <section id="partner" className="section-padding relative bg-section-dark">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionHeading
          ...
          title={<>エムスタ上で、<span className="bg-gradient-to-r from-accent-400 to-primary-400 bg-clip-text text-transparent">アプリビジネス</span>を始める。</>}
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-12">
          ...
          <div className="lg:col-span-5">
            <div className="rounded-3xl border border-white/20 bg-white/5 p-7 text-white shadow-xl md:p-8">
              <p className="text-xs font-bold uppercase tracking-widest text-accent-400">Lead capture</p>
          ...
          <ul className="grid divide-y divide-white/10 sm:grid-cols-5 sm:divide-x sm:divide-y-0">
            {ranks.map((r) => (
              <li key={r.name} className="flex items-center justify-between gap-4 px-5 py-5 sm:flex-col sm:items-start">
                ...
                <p className="text-xl font-black text-accent-400 sm:mt-2">{r.rate}</p>
              </li>
            ))}
          </ul>
        </motion.div>
  ...
  return (
    <div className="card-on-dark grid gap-6 rounded-3xl p-7 shadow-xl md:grid-cols-12 md:p-8">
      <div className="md:col-span-7">
        ...
        <p className="mt-3 text-sm leading-relaxed text-on-dark-muted md:text-base">{description}</p>
      </div>
