...
export const CTA_LINKS = {
  ...
  spirOfficial: resolveLink(cmsContact.spirOfficial, process.env.NEXT_PUBLIC_SPIR_OFFICIAL_URL),
  spirThreeHour: resolveLink(cmsContact.spirThreeHour, process.env.NEXT_PUBLIC_SPIR_3H_URL),
  spirFull: resolveLink(cmsContact.spirFull, process.env.NEXT_PUBLIC_SPIR_FULL_URL),
  spirPartner: resolveLink(cmsContact.spirPartner, process.env.NEXT_PUBLIC_SPIR_PARTNER_URL),
} as const;

/**
 * 一般相談カレンダーのSpir埋込コード（/keystatic で編集）。未設定時は空文字。
 */
export const GENERAL_CALENDAR_EMBED = cmsContact.generalCalendarEmbed?.trim() ?? "";
